...
void Camera::update(Renderer* lpRenderer)
{
	XMFLOAT3 front;
	...
	BufferPerFrame cbPerFrame;
	ZeroMemory(&cbPerFrame, sizeof(cbPerFrame));
	cbPerFrame.viewMatrix = mViewMatrix;
	lpRenderer->updateFrameConstantBuffer(&cbPerFrame);
}
...
void Camera::sendToShader(Renderer* lpRenderer)
{
	BufferPerApp cbPerApp;
	ZeroMemory(&cbPerApp, sizeof(cbPerApp));
	cbPerApp.projMatrix = mProjMatrix;
	lpRenderer->updateAppConstantBuffer(&cbPerApp);
}
